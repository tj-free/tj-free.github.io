...
fn vertexMain(@location(0) pos: vec2f, @builtin(instance_index) idx: u32) -> @builtin(position) vec4f {
  ...
  let u = idx % 20; // we are expecting 10x10, so modulo 10 to get the x index
  let v = idx / 20; // divide by 10 to get the y index
  let uv = vec2f(f32(u), f32(v)) / 20; // normalize the coordinates to [0, 1]
  let halfLength = 1.f; // half cell length
  let cellLength = halfLength * 2.f; // full cell length
  let cell = pos / 20; // divide the input quad into 10x10 pieces
