...
export default class CameraLineStrip2DAliveDeadObject extends SceneObject {
  ...
  async createGeometry() {
    ...
    this._cellStatus = new Uint32Array(this.gridSize * this.gridSize); 
  ...
  render(pass) {
    ...
    pass.draw(this._vertices.length / 2, this.gridSize * this.gridSize);  // number of vertices to draw and number of instances to draw (100 here)
  }
  ...
  compute(pass) { 
    ...
    pass.setBindGroup(0, this._bindGroups[this._step % 2]);     // bind the uniform buffer
    pass.dispatchWorkgroups(Math.ceil(this.gridSize / 4), Math.ceil(this.gridSize / 4)); // sending how many instances to compute for each work group
    ++this._step;
  }
}