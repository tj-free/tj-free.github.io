...
// Returns vec2f(t, 1.0) if hit, vec2f(t, -1.0) otherwise
fn sphereRayHitCheck(s: vec3f, d: vec3f, ct: f32) -> vec2f {
  let a = dot(d, d);
  let b = 2.0 * dot(d, s);
  let c = dot(s, s) - 1.0; // because unit sphere centered at origin: x² + y² + z² = 1

  let discriminant = b * b - 4.0 * a * c;

  if (discriminant < 0.0) {
    return vec2f(ct, -1.0); // No real roots → no intersection
  }
  ...
  if (t < 0.0) {
    return vec2f(ct, -1.0); // Ray hits the sphere behind the camera
  }

  if (ct < 0.0 || t < ct) {
    return vec2f(t, 1.0); // Valid hit, and it’s closer than any previous hit
  }

  return vec2f(ct, -1.0); // Keep the previous closer hit
}
...
fn computeProjectiveMain(@builtin(global_invocation_id) global_id: vec3u) {
  ...
  if (uv.x < texDim.x && uv.y < texDim.y) {
    ...
    // Compute the intersection with the objects in the scene
    var hitInfo = rayBoxIntersection(spt, rdir);
    
    // Assign color based on hit information
    assignColor(uv, hitInfo.x, i32(hitInfo.y));
  }
}