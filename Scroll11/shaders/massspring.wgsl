...
fn computeMain(@builtin(global_invocation_id) global_id: vec3u) {
  ...
  if (idx < arrayLength(&springsIn)) {
    ...
    var vA = particlesIn[aIdx].dv; // delta v a 
    var vB = particlesIn[bIdx].dv; // delta v b
    ...
    let dir = normalize(diff); // a direction from ptA to pt B
    if (massA > 0) {
      if (massA > pow(2.0,32.0)) {
        vA += (force * dir) / (pow(2.0,32.0) * 1000); // mass in grams
      } else {
        vA += (force * dir) / (massA * 1000);
      }
    }
    if (massB > 0) {
      if (massB > pow(2.0,32.0)) {
        vB -= (force * dir) / (pow(2.0,32.0) * 1000); // opposite dir
      } else {
        vB -= (force * dir) / (massB * 1000);
      }
    }
