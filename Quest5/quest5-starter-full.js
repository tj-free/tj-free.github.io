...
class PolygonObject extends SceneObject{constructor(device,canvasFormat,filename){super(device,canvasFormat);this._polygon=new Polygon(filename);this._index=0;}
...
render(pass){pass.setPipeline(this._renderPipeline);pass.setVertexBuffer(0,this._vertexBuffer);pass.setBindGroup(0,this._bindGroup);pass.draw(this._numV);console.log(this._inside);this._device.queue.writeBuffer(this._windingNumberBuffer,0,new Int32Array(1));}
...
compute(pass){pass.setPipeline(this._computePipeline);pass.setBindGroup(0,this._bindGroup);pass.dispatchWorkgroups(Math.ceil(this._numV/256));}}
...
async function init(){const canvasTag=document.createElement('canvas');canvasTag.id="renderCanvas";document.body.appendChild(canvasTag);const renderer=new Renderer(canvasTag);await renderer.init();const polygon=new PolygonObject(renderer._device,renderer._canvasFormat,'/assets/box.polygon');await renderer.appendSceneObject(polygon);let fps='??';var fpsText=new StandardTextObject('fps: '+fps,'10');let insideInit='unknown'
var InfoText=new StandardTextObject('Inside: '+insideInit+'\nw/W: change polygon');canvasTag.addEventListener('mousemove',async(e)=>{var mouseX=(e.clientX/window.innerWidth)*2-1;var mouseY=(-e.clientY/window.innerHeight)*2+1;const mouse=new Float32Array([mouseX,mouseY]);polygon.updateMouseBuffer(mouse)
var inside=await polygon.checkWindingNumber();InfoText.updateText('Inside: '+inside+'\nw/W: change polygon');});var needToWait=false;window.addEventListener("keydown",async(e)=>{switch(e.key){case'w':case'W':if(!needToWait){needToWait=true;const polygonTypes=["box.polygon","circle.polygon","star.polygon","human.polygon","dense.polygon"];polygon._index+=1;if(polygon._index>=polygonTypes.length){polygon._index=0;}
await polygon.changePolygon('/assets/'+polygonTypes[polygon._index]);needToWait=false;}
