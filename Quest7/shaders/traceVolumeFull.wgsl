...
fn depthEncoding(t0: f32, t1: f32, tmin: f32, tmax: f32) -> vec4f {
  let r = vec4f(1.0, 0.0, 0.0, 1.0);  // Red color
  let g = vec4f(0.0, 1.0, 0.0, 1.0);  // Green color
  let b = vec4f(0.0, 0.0, 1.0, 1.0);  // Blue color
  let t = (t1 + t0) * 0.5;            // Average depth
  let m = (tmax + tmin) * 0.5;        // Middle point between min and max depths

  if (t > m) {  // Interpolating between green and blue
    let lambda = (t - m) / (tmax - m);
    return b * lambda + g * (1.0 - lambda);
  }

  let lambda = (m - t) / (m - tmin);
  return r * lambda + g * (1.0 - lambda);
}
...
fn traceSceneMIP(uv: vec2i, p: vec3f, d: vec3f) {
  ...
  if (hits.x >= 0) { 
    ...
    var hitVal = hits.x + 1e-4;

    let voxelSize = vec3f(1, 1, 1) * volInfo.sizes.xyz / max(max(volInfo.dims.x, volInfo.dims.y), volInfo.dims.z); //normalized voxel size
    var intensity = 0.0;

    // Ray marching
    // while still inside volume
    while (hitVal < hits.y) {
      var point = p + d * hitVal;
      
      // compute voxel index with voxel size
      let halfsize = volInfo.dims * volInfo.sizes * 0.5 / max(max(volInfo.dims.x, volInfo.dims.y), volInfo.dims.z);
      let voxelIndex = vec3i((point + halfsize.xyz) / voxelSize);

      // get voxel value
      let i = voxelIndex.z * (i32(volInfo.dims.x) * i32(volInfo.dims.y)) + voxelIndex.y * i32(volInfo.dims.x) + voxelIndex.x;
      let voxVal = volData[i];

      // set current intensity
      intensity = max(intensity,voxVal);

      // compute voxel corners
      let minCorner = vec3f(voxelIndex) * voxelSize - halfsize.xyz;
      let maxCorner = minCorner + voxelSize;

      // use corners to find next hitVal
      // Compute next hit values in x, y, z using the voxel faces
      let nextHitX = getNextHitValue(hits.x+1e-4, hitVal, point.x, vec2f(minCorner.y,minCorner.z), vec2f(maxCorner.y,maxCorner.z), p.x, d.x, p.yz, d.yz);
      let nextHitY = getNextHitValue(hits.x+1e-4, hitVal, point.y, vec2f(minCorner.x,minCorner.z), vec2f(maxCorner.x,maxCorner.z), p.y, d.y, p.xz, d.xz);
      let nextHitZ = getNextHitValue(hits.x+1e-4, hitVal, point.z, vec2f(minCorner.x,minCorner.y), vec2f(maxCorner.x,maxCorner.y), p.z, d.z, p.xy, d.xy);

      // Set the next hit value to the minimum of the three
      hitVal = min(nextHitX, min(nextHitY, nextHitZ)) + 1e-4;
    }

    // set color based on intensity
    intensity = intensity / 4095; // max is 4095 - adjust this for other vol data
...
// a function to trace the volume - volume rendering
fn traceSceneXRay(uv: vec2i, p: vec3f, d: vec3f) {
  // find the start and end point
  var hits = rayVolumeIntersection(p, d);
  var color = vec4f(0.f/255, 0.f/255, 0.f/255, 1.); 
  ...
  if (hits.x >= 0) { 
    ...
    while (hitVal < hits.y) {
      ...
      // Set the next hit value to the minimum of the three
      let nextHit = min(nextHitX, min(nextHitY, nextHitZ)) + 1e-4;

      let deltaT = nextHit - hitVal + 1e-4;

      attenuation = attenuation + (deltaT * voxVal / 4095);

      hitVal = nextHit + 1e-4;
    }

    // set color based on attenuation
    let intensity = clamp(1 - exp(-attenuation),0.0,1.0);
    color = vec4f(intensity, intensity, intensity, 1.0);
...
fn traceSceneBlend(uv: vec2i, p: vec3f, d: vec3f) {
  // find the start and end point
  var hits = rayVolumeIntersection(p, d);
  var color = vec4f(0.f/255, 0.f/255, 0.f/255, 1.); 
  ...
  if (hits.x >= 0) { 
    ...
    while (hitVal < hits.y) {
      ...
      color = color + (deltaT * voxVal / 4095 * depthEncoding(hitVal,nextHit,hits.x,hits.y));
...
fn computeProjectiveMain(@builtin(global_invocation_id) global_id: vec3u) {
  if (mode == 0) {
    ...
    if (uv.x < texDim.x && uv.y < texDim.y) {
      ...
      // Projective camera ray: start at (0, 0, 0) and go through the pixel center used in orthogonal
    ...
    if (uv.x < texDim.x && uv.y < texDim.y) {
      // compute the pixel size
      let psize = cameraPose.focal * vec2f(2, 2) / cameraPose.res.xy;
      ...
      // (?x * pixel size in x + ?y * pixel size in y)

      // Apply transformation (camera and object pose)
      spt = transformPt(spt);  // Although the origin is fixed at (0, 0, 0), this is just for consistency
      rdir = transformDir(rdir);
